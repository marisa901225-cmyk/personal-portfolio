...
import { MemoryChat } from '../features/memories/MemoryChat';
...
export const MemoriesPage: React.FC = () => {
    ...
    return (
        <div className="space-y-6 pb-20 md:pb-0">
            ...
            <div className="bg-white rounded-3xl p-4 md:p-6 shadow-sm border border-slate-100 sticky top-0 md:relative z-10">
                <div className="flex flex-col gap-4">
                    {/* View Switcher & Search */}
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="flex p-1 bg-slate-100 rounded-2xl md:w-fit">
                            <button
                                onClick={() => setViewMode('grid')}
                                className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${viewMode === 'grid'
                            ...
                            <button
                                ...
                                className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${viewMode === 'chat'
                                        ? 'bg-white text-indigo-600 shadow-sm'
                                        : 'text-slate-500 hover:text-slate-700'
                                    }`}
                            >
                    ...
                    {/* Categories - Only show in grid mode */}
                    {viewMode === 'grid' && (
                        <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar -mx-2 px-2">
                            {CATEGORIES.map(({ value, label }) => (
                                <button
                                    ...
                                    className={`px-5 py-2.5 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${selectedCategory === value
            ...
            {/* Content Area */}
            {viewMode === 'chat' ? (
                apiClient && <MemoryChat apiClient={apiClient} />
            ) : (
                <>
                    {/* Grid View */}
                    {loading ? (
