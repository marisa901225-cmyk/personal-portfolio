        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col h-full">
                    ...
                    {historyStats && (
                        <div className="mt-5 pt-4 border-t border-slate-100 flex-1 flex flex-col justify-end">
                            <div className="grid grid-cols-2 gap-y-4 gap-x-8">
                                <div>
                                    <span className="text-xs text-slate-500 block mb-1">시작 금액</span>
                                ...
                                <div>
                                    <span className="text-xs text-slate-500 block mb-1">변동 금액</span>
                                    <div className={`flex items-center gap-1 text-sm font-bold ${historyStats.change >= 0 ? 'text-red-500' : 'text-blue-500'}`}>
                                        {historyStats.change >= 0 ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
                                        {historyStats.change > 0 ? '+' : ''}{formatCurrency(historyStats.change)}
                                    </div>
                                </div>
                                <div>
                                    <span className="text-xs text-slate-500 block mb-1">수익률</span>
                                    <div className={`flex items-center gap-1 text-sm font-bold ${historyStats.changeRate >= 0 ? 'text-red-500' : 'text-blue-500'}`}>
