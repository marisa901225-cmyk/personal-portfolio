...
| 3 | PROMPT-003 | Add manual snapshot capture from the dashboard | P3 | ⬜ Pending |
| 4 | PROMPT-004 | Export trade history to CSV | P3 | ⬜ Pending |
...
### [PROMPT-003] Add manual snapshot capture from the dashboard

**⏱️ Execute this prompt now, then proceed to PROMPT-004**

> **🚨 REQUIRED: Use `replace_string_in_file` or `create_file` to make changes. Do NOT just show code.**

**Task**: Add a manual “Save Snapshot” action in the dashboard and refresh history after saving.  
**Files to Modify**: `backendClient.ts`, `hooks/usePortfolio.ts`, `components/Dashboard.tsx`, `components/DashboardSummary.tsx`, `App.tsx`

#### Instructions:

1. Add a `createSnapshot()` method to `ApiClient` that calls `POST /api/portfolio/snapshots`.
2. Expose a `createSnapshot()` action in `usePortfolio` that calls the backend and refreshes history.
3. Add a snapshot button in `DashboardSummary` and wire it through `Dashboard` + `App`.

#### Implementation Code:

File: `backendClient.ts` (insert inside `ApiClient` after `fetchSnapshots`)
```typescript
...
File: `hooks/usePortfolio.ts` (update interface, add function, update return object)
```typescript
...
File: `components/Dashboard.tsx` (imports, props, snapshot handler, and summary props)
```typescript
...
```typescript
      <DashboardSummary
        ...
        onCreateSnapshot={onCreateSnapshot ? handleCreateSnapshot : undefined}
        isSnapshotSaving={isSnapshotSaving}
        realEstateSummary={realEstate}
      />
```

File: `components/DashboardSummary.tsx`
```typescript
        ...
        <div className="flex justify-between items-start z-10 relative">
          ...
          {onCreateSnapshot && (
            <button
              ...
              className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs border border-slate-200 text-slate-600 hover:border-indigo-400 hover:text-indigo-600 disabled:opacity-60 disabled:cursor-not-allowed"
              title="스냅샷 저장"
            >
              <RefreshCw size={14} className={isSnapshotSaving ? 'animate-spin' : ''} />
              스냅샷 저장
            </button>
          )}
        </div>
