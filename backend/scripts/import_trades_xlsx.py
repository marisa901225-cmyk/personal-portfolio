...
def main() -> int:
    parser = argparse.ArgumentParser(description="Import trades from combined_statements_valuation.xlsx")
    parser.add_argument(
        "--xlsx",
        default=str(ROOT_DIR / "combined_statements_valuation.xlsx"),
        help="Path to the normalized XLSX (default: combined_statements_valuation.xlsx)",
    )
    parser.add_argument(
        "--sheet",
        default=DEFAULT_SHEET,
        help=f"Sheet name to import (default: {DEFAULT_SHEET})",
    )
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Delete existing trades before import.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.sheet not in sheet_map:
        ...
        print("Available sheets:", ", ".join(sheet_map.keys()), file=sys.stderr)
        return 1

    rows = iter_rows(xlsx_path, sheet_map[args.sheet], shared_strings)
    ...
    try:
        ...
        if args.replace:
            session.query(Trade).filter(Trade.user_id == user.id).delete()
        ...
        if stats["assets_created"] > 0:
            for asset_id, last_ts in created_last_ts.items():
                asset = session.get(Asset, asset_id)
                if asset and asset.deleted_at is None:
                    asset.deleted_at = last_ts
        ...
        print(f"- eligible buy/sell rows: {stats['eligible']}")
        print(f"- inserted trades: {stats['inserted']}")
        print(f"- duplicates skipped: {stats['duplicates']}")
        print(f"- assets created: {stats['assets_created']}")
        print(f"- missing name: {stats['missing_name']}")
        print(f"- invalid qty/price: {stats['invalid_values']}")
        print(f"- missing date: {stats['missing_date']}")
        print(f"- duplicate asset names: {duplicate_assets}")
