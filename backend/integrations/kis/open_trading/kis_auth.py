...
def _throttle_rest() -> None:
    while True:
        with _rest_rate_lock:
            ...
            while _rest_rate_timestamps and now - _rest_rate_timestamps[0] >= _REST_RATE_WINDOW:
                _rest_rate_timestamps.popleft()
            if len(_rest_rate_timestamps) < _REST_RATE_LIMIT:
                _rest_rate_timestamps.append(now)
                return
            sleep_for = _REST_RATE_WINDOW - (now - _rest_rate_timestamps[0])
        if sleep_for > 0:
