...
def test_day_entry_does_not_take_profit_buffer_before_swing_uses_it(tmp_path) -> None:
    asof = "20260410"
    api = FakeAPI()
...
def test_day_entry_uses_profit_buffer_only_after_swing_budget_leaves_room(tmp_path) -> None:
    ...
    api._quotes["005930"] = {"price": 50_000, "change_pct": 1.5}
    api._positions = [{"code": "SWING01", "qty": 1, "avg_price": 800_000.0}]
    ...
    bot = HybridTradingBot(api, config=cfg)
    bot.state.trade_date = asof
    bot.state.realized_pnl_total = 100_000.0
    bot.state.open_positions["SWING01"] = PositionState(
        type="S",
        entry_time="2026-04-10T09:05:00",
...
def test_day_entry_caps_large_reused_swing_budget_per_symbol(tmp_path) -> None:
    ...
    api._quotes["005930"] = {"price": 50_000, "change_pct": 1.5}
    api._positions = [{"code": "SWING01", "qty": 10, "avg_price": 50_000.0}]
...
def test_conditional_day_budget_splits_across_cash_supported_slots(tmp_path) -> None:
    asof = "20260410"
    api = FakeAPI()
    api._cash_available = 400_000
    api._quotes["005930"] = {"price": 50_000, "change_pct": 1.5}
    api._positions = [{"code": "SWING01", "qty": 1, "avg_price": 600_000.0}]
    ...
    bot.state.open_positions["SWING01"] = PositionState(
        type="S",
        entry_time="2026-04-10T09:05:00",
        entry_price=600_000.0,
        qty=1,
        highest_price=600_000.0,
        entry_date="20260410",
        bars_held=0,
    )
...
def test_conditional_day_budget_keeps_one_slot_when_swing_budget_is_full(tmp_path) -> None:
    asof = "20260410"
    api = FakeAPI()
