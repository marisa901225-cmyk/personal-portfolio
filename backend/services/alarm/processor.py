...
from .filters import mask_sensitive_info, is_spam, is_promo_spam, is_whitelisted, should_ignore, is_spam_llm, is_review_spam
...
def create_spam_alarm(alarm, classification: str, reason: str, masked_text: str) -> SpamAlarm:
    return SpamAlarm(
        ...
        discard_reason=reason, rule_version=1
    )
...
async def process_alarms_batch(db: Session):
    ...
    recent_texts = {r[0] for r in db.query(IncomingAlarm.raw_text).filter(IncomingAlarm.status.in_(["processed", "discarded"]), IncomingAlarm.received_at >= ten_min_ago).all()}
    ...
    for alarm in deduplicated:
        ...
        if "%antext" in original or "%evtprm" in original:
            ...
            if not is_whitelisted(full_check):
                if is_review_spam(full_check):
                    ...
                    if is_s: discard_reason, classification = "Spam rule match", cl
                    elif is_promo_spam(full_check, db): discard_reason, classification = "Promotion rule match", "promo_rule"
                    else:
                        is_s_l, cl_l = is_spam_llm(full_check)
                        if is_s_l: discard_reason, classification = "LLM spam classification", cl_l
                
                if discard_reason: filtered_reasons["광고/프로모션"] += 1
        ...
        if alarm.sender: senders.add(alarm.sender)
        
        # 카드 승인 처리
        card = parse_card_approval(original)
        if card:
            cat = nb_pipeline.predict([card["merchant"]])[0] if nb_pipeline and card["merchant"] else "기타"
            db.add(Expense(user_id=user.id, date=card["date"], amount=card["amount"], merchant=card["merchant"], method=card["method"], category=cat))
    ...
    if to_analyze_expenses:
        ...
        if insight: summaries.insert(0, f"💰 <b>지출 분석</b>: {html.escape(insight)}\n")
    ...
    if summaries:
        title = "알림 리포트"
        if to_analyze_expenses: title = "알림 및 가계부 리포트" if to_summarize_alarms else "가계부 리포트"
        elif not to_summarize_alarms: title = ["랜덤 토픽", "잡학사전", "오늘의 TMI", "심심풀이 지식"][datetime.now().minute // 5 % 4]
