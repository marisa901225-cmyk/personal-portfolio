...
def _is_tailnet_or_local_request(request: Request) -> bool:
    origin_ip = _request_origin_ip(request)
    return _is_local_ip(origin_ip) or _is_tailscale_ip(origin_ip)
...
async def verify_api_token(
    ...
    """
    통합 인증 로직.
    1. 유효한 JWT는 모든 접근 경로에서 통과.
    2. X-API-Token 단독 인증은 Tailnet/localhost 원점에서만 통과.
    """
    # 1. JWT 검증 시도
    token = auth_token
    ...
    tailnet_or_local = _is_tailnet_or_local_request(request)

    if jwt_valid:
        return

    if tailnet_or_local and api_key_valid:
        return
    ...
    raise HTTPException(
        ...
        detail=(
            ...
            if tailnet_or_local
            else "Invalid authentication credentials (JWT required)"
        ),
    )
